...
//***************************************************************************************************	
	// Spring Security should completely ignore URLs starting with /authenticate and
	// /h2-console/
	// add the url need to ignore the security process inside the antMatcher
	@Override
	public void configure(WebSecurity web) throws Exception {
		web.ignoring().antMatchers("/login","/testing", "/h2-console/**", "/validate");
	}

	@Override
	protected void configure(HttpSecurity httpSecurity) throws Exception {
		httpSecurity.csrf().disable().authorizeRequests().antMatchers("/login","/testing", "/h2-console/**", "/validate")
